                ...
                for ms in consumer.poll().unwrap().iter() {
                    for m in ms.messages() {
                        match std::str::from_utf8(m.value) {
                            Ok(content) => {
                                match EventUber::from_json_string(content) {
                                    Ok(message) => {
                                    ...
                                    Err(error) => {
                                        let error_message: String = format!("Cannot convert uber event message: {}", error);
                            ...
                            Err(error) => {
                                let error_message: String = format!("Cannot read uber event message: {}", error);
                                logger::error("event_consumer", "consume_uber", &error_message);
                            }
                        }
                    }
