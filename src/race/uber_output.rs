    ...
    pub fn take_passenger_and_drive_to(&self, input_file_path: &str) {
        match &self {
            UberOutput::Local { path } => {
                println!("Should copy file locally from {} to {}", input_file_path, path);
            }

            UberOutput::Sftp { login, authentication_method, remote_path } => {
                println!("Should copy file remotely from {} to {} using login {} with auth method {:?}",
