...
pub fn copy(input: &Path, output: &UberOutput) -> Result<bool, RaceActionError> {
    match output {
        UberOutput::Local { path } => {
            let file_name = input.file_name().ok_or(RaceActionError::FileName(path.to_string()))?;
            let file_name = file_name.to_str().ok_or(RaceActionError::FileName(path.to_string()))?;
            let passenger_name: String = file_name.replace('"', "");
            let full_destination_path: String = format!("{}/{}", path, passenger_name);

            fs::copy(input, &full_destination_path)
                .map_err(|error| {
