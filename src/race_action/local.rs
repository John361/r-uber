                    ...
                    match file_name.to_str() {
                        Some(name) => {
                            ...
                            if let Err(error) = std::fs::copy(input, &full_destination_path) {
                                let error_message: String = format!("Cannot copy file {:?} to {}: {}", input, &full_destination_path, error);
                                logger::warn("local", "copy", &error_message);
                                Err(error_message)
                            } else {
                                let success_message: String = format!("Successfully copy file from {:?} to {}", input, &full_destination_path);
